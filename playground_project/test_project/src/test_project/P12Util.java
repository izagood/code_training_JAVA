...
class P12Util {
	...
	/**
	 * Returns the first private key entry found in the given keystore. If more than
	 * one private key is present, the key that is returned is undefined.
	 *
	 * @param p12InputStream an input stream for a PKCS#12 keystore
	 * @param password       the password to be used to load the keystore and its
	 *                       entries; may be blank, but must not be {@code null}
	 *
	 * @return the first private key entry found in the given keystore
	 *
	 * @throws KeyStoreException if a private key entry could not be extracted from
	 *                           the given keystore for any reason
	 * @throws IOException       if the given input stream could not be read for any
	 *                           reason
	 */
	public static PrivateKeyEntry getFirstPrivateKeyEntryFromP12InputStream(final InputStream p12InputStream,
